  ...
  return (
    ...
    >
      {!showParamsForm ? (
        ...
        <div className="lg:col-span-1 p-6 rounded-2xl bg-card border border-border backdrop-blur-xl shadow-xl flex flex-col justify-between min-h-[480px]">
          {(() => {
            ...
            return (
              <>
                  ...
                  {isRunning ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <div className="relative w-20 h-20 mb-6 flex items-center justify-center">
                        ...
                        <div className="w-12 h-12 bg-background rounded-full border border-primary flex items-center justify-center">
                          <RefreshCw className="w-6 h-6 text-primary animate-spin" style={{ animationDuration: '4s' }} />
                        </div>
                      </div>
                      <h3 className="text-lg font-bold text-foreground mb-1">Scrape Campaign Active</h3>
                      ...
                      <div className="text-[10px] uppercase font-semibold text-muted-foreground bg-background/80 py-1.5 px-3 rounded-lg border border-border">
                        Target Storefront: <span className="text-zinc-700 dark:text-zinc-300 font-mono font-bold ml-1">{latestJob?.params?.country?.toUpperCase()} / {latestJob?.params?.lang?.toUpperCase()}</span>
                      </div>
                    </div>
                  ) : (
                    ...
                    <div className="flex flex-col items-center">
                      <div className="relative w-20 h-20 mb-6 flex items-center justify-center">
                        {latestJob?.status === "completed" ? (
                          <>
                            ...
                            <div className="w-12 h-12 bg-background rounded-full border border-emerald-500 flex items-center justify-center">
                          ...
                          <>
                            ...
                            <div className="w-12 h-12 bg-background rounded-full border border-rose-500 flex items-center justify-center">
                      ...
                      <p className="text-xs text-muted-foreground max-w-[200px] mb-4 text-center">
                        {latestJob?.status === "completed"
                      ...
                      <div className="bg-background/60 p-3 rounded-xl border border-border w-full mb-2">
                        <span className="text-[10px] text-muted-foreground block uppercase font-bold tracking-wider mb-1">Scraped Target</span>
                        <span className="text-xs font-mono font-bold text-zinc-700 dark:text-zinc-300 capitalize">{latestJob?.target}</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2.5 mt-auto pt-6 border-t border-border w-full">
                  {!isRunning && (
                    <button
                      onClick={() => {
                        if (latestJob?.job_id) {
        ...
        <div className="lg:col-span-1 p-6 rounded-2xl bg-card border border-border backdrop-blur-xl shadow-xl flex flex-col justify-between">
          <div>
            {!isAdvancedMode ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold">Production Fetchers</div>
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        activePreset === "all-products"
                          ...
                          : "bg-muted/30 border border-border hover:bg-muted/30 hover:border-border"
                      )}
                    >
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-3 h-3 rounded-full border flex items-center justify-center transition-all duration-300",
                          activePreset === "all-products" ? "border-primary" : "border-zinc-300 dark:border-zinc-700"
                        )}>
                          {activePreset === "all-products" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                        </div>
                        <span className="text-xs font-semibold text-foreground">Fetch All Products</span>
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-1 font-normal leading-normal">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-3 h-3 rounded-full border flex items-center justify-center transition-all duration-300",
                          activePreset === "all-brands" ? "border-primary" : "border-zinc-300 dark:border-zinc-700"
                        )}>
                          {activePreset === "all-brands" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                        </div>
                        <span className="text-xs font-semibold text-foreground">Fetch All Brands</span>
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-1 font-normal leading-normal">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-3 h-3 rounded-full border flex items-center justify-center transition-all duration-300",
                          activePreset === "all-categories" ? "border-primary" : "border-zinc-300 dark:border-zinc-700"
                        )}>
                          {activePreset === "all-categories" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                        </div>
                        <span className="text-xs font-semibold text-foreground">Fetch All Categories</span>
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-1 font-normal leading-normal">
                ...
                <div className="space-y-2 pt-2">
                  <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold">Testing Fetchers</div>
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-3 h-3 rounded-full border flex items-center justify-center transition-all duration-300",
                          activePreset === "first-n-categories" ? "border-primary" : "border-zinc-300 dark:border-zinc-700"
                        )}>
                          {activePreset === "first-n-categories" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                        </div>
                        <span className="text-xs font-semibold text-foreground">Fetch first N categories</span>
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-1 font-normal leading-normal">
                      ...
                      {activePreset === "first-n-categories" && (
                        <div className="mt-2.5 flex items-center gap-2 self-start" onClick={(e) => e.stopPropagation()}>
                          <span className="text-[10px] text-muted-foreground">Categories (N):</span>
                          <div className="flex items-center bg-background border border-border rounded-lg h-7">
                            <button 
                              type="button"
                              onClick={() => setPresetN(Math.max(1, presetN - 1))}
                              className="w-7 h-7 flex items-center justify-center text-muted-foreground hover:text-foreground text-xs font-bold transition-all hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-l-lg"
                            >
                              −
                            </button>
                            <input 
                              ...
                              className="w-10 text-center bg-transparent border-0 text-foreground text-xs font-semibold focus:ring-0 focus:outline-none p-0 h-full"
                            />
                            <button 
                              type="button"
                              onClick={() => setPresetN(presetN + 1)}
                              className="w-7 h-7 flex items-center justify-center text-muted-foreground hover:text-foreground text-xs font-bold transition-all hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-r-lg"
                            >
                              +
                            </button>
                          </div>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-3 h-3 rounded-full border flex items-center justify-center transition-all duration-300",
                          activePreset === "n-pages-of-n-categories" ? "border-primary" : "border-zinc-300 dark:border-zinc-700"
                        )}>
                          {activePreset === "n-pages-of-n-categories" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                        </div>
                        <span className="text-xs font-semibold text-foreground">Fetch N pages of N categories</span>
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-1 font-normal leading-normal">
                      ...
                      {activePreset === "n-pages-of-n-categories" && (
                        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-muted-foreground">Pages (P):</span>
                            <div className="flex items-center bg-background border border-border rounded-lg h-7">
                              <button 
                                type="button"
                              ...
                              <input 
                                ...
                                className="w-8 text-center bg-transparent border-0 text-foreground text-xs font-semibold focus:ring-0 focus:outline-none p-0 h-full"
                              />
                              <button 
                                type="button"
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-3 h-3 rounded-full border flex items-center justify-center transition-all duration-300",
                          activePreset === "resource-stats" ? "border-primary" : "border-zinc-300 dark:border-zinc-700"
                        )}>
                          {activePreset === "resource-stats" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                        </div>
                        <span className="text-xs font-semibold text-foreground">Fetch Resource Stats</span>
                      </div>
                      <p className="text-[10px] text-muted-foreground mt-1 font-normal leading-normal">
                ...
                <div className="space-y-3 pt-3 border-t border-border">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="block text-[10px] text-muted-foreground font-semibold mb-1">Country Storefront</Label>
                      <Select value={country} onValueChange={setCountry}>
                        <SelectTrigger className="w-full h-8 bg-background/50 border border-border text-foreground text-xs">
                          <SelectValue placeholder="Country" />
                        </SelectTrigger>
                        <SelectContent className="bg-background border border-border text-foreground text-xs">
                    ...
                    <div>
                      <Label className="block text-[10px] text-muted-foreground font-semibold mb-1">Target Language</Label>
                  ...
                  {activePreset && ["all-products", "first-n-categories", "n-pages-of-n-categories"].includes(activePreset) && (
                    <div className="animate-in fade-in duration-300">
                      <Label className="block text-[10px] text-muted-foreground font-semibold mb-1">Image Processing Strategy</Label>
                  ...
                  {activePreset && ["all-products", "first-n-categories", "n-pages-of-n-categories"].includes(activePreset) && (
                    ...
                    >
                      <Checkbox
                        ...
                        className="border border-border bg-background w-4 h-4 mt-0.5 data-[state=checked]:bg-primary-dark data-[state=checked]:border-primary-dark"
                      />
                      <Label
                        htmlFor="preset-use-current-db"
                        className="flex flex-col gap-0.5 leading-tight cursor-pointer text-foreground"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <div className="w-full text-xs font-semibold transition-colors">Use Current DB</div>
                        <p className="text-[10px] text-muted-foreground font-normal">Use current stored database as local cache to avoid fetching duplicates</p>
                      </Label>
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                {/* Scrape Target */}
                <div className="mb-4">
                  <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Scraping Target</Label>
                  <Select value={target} onValueChange={setTarget}>
                    <SelectTrigger className="w-full h-9 bg-background/50 border border-border text-foreground">
                ...
                {!statsOnly && target === "products" && (
                  <div className="mb-4 animate-in fade-in duration-200">
                    <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Image Processing Strategy</Label>
                    <Select value={crawlMode} onValueChange={setCrawlMode}>
                      <SelectTrigger className="w-full h-9 bg-background/50 border border-border text-foreground">
                ...
                {!statsOnly && target === "products" && (
                  <div className="mb-4 space-y-4 animate-in fade-in duration-200">
                    <div>
                      <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Category Scope</Label>
                      <Select 
                        value={categoryScope} 
                    ...
                    {categoryScope === "custom" && (
                      <div className="animate-in slide-in-from-top-2 duration-200">
                        <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Custom Category Path</Label>
                        <Input
                          type="text"
                          value={customPath}
                          onChange={(e) => {
                          ...
                          className="w-full h-9 bg-background/50 border border-border text-foreground placeholder:text-muted-foreground"
                        />
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Country Storefront</Label>
                    <Select value={country} onValueChange={setCountry}>
                      <SelectTrigger className="w-full h-9 bg-background/50 border border-border text-foreground">
                        <SelectValue placeholder="Country" />
                      </SelectTrigger>
                  ...
                  <div>
                    <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Target Language</Label>
                    <Select value={lang} onValueChange={setLang}>
                      <SelectTrigger className="w-full h-9 bg-background/50 border border-border text-foreground">
                        <SelectValue placeholder="Language" />
                      </SelectTrigger>
                ...
                {!statsOnly && target === "products" && (
                  <div className="mb-6 animate-in fade-in duration-200">
                    <Label className="block text-xs text-muted-foreground font-semibold mb-1.5">Pages Range</Label>
                ...
                {!statsOnly && target === "products" && (
                  <div className="mb-6 animate-in fade-in duration-200">
                    <div className="flex justify-between items-center mb-1.5">
                      <Label className="text-xs text-muted-foreground font-semibold">Concurrency Control</Label>
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-background text-emerald-400 font-bold border border-border">
                        {workers} Threads
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="w-full accent-primary h-1 bg-background rounded-lg cursor-pointer appearance-none border border-border"
                      />
                    </div>
                ...
                <div className="space-y-3 pb-4">
                  {!statsOnly && target === "products" && (
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="text-[10px] text-muted-foreground font-normal">Fetch detail specs, HTML overview, multiple images</p>
                      </Label>
                    </div>
                  )}
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      (target === "products" && crawlMode === "catalog") || 
                      (target !== "products" && !includeMedia)
                        ? "bg-muted/20 border-border text-muted-foreground/70 cursor-not-allowed opacity-50"
                        : download
                          ? "bg-primary-dark/[0.04] border-primary-dark/50 shadow-md shadow-primary-deep/5 cursor-pointer"
                          : "bg-muted/30 border border-border hover:bg-muted/30 hover:border-border cursor-pointer"
                    )}
                  >
                    <Checkbox
                      id="download-media"
                      ...
                      onClick={(e) => e.stopPropagation()}
                      className="border border-border bg-background w-4 h-4 mt-0.5 data-[state=checked]:bg-primary-dark data-[state=checked]:border-primary-dark disabled:opacity-40"
                    />
                    <Label
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground font-normal">Download and categorize image assets locally</p>
                    </Label>
                  </div>
                  ...
                  >
                    ...
                    <Label htmlFor="stats-only" className="flex flex-col gap-0.5 cursor-pointer leading-tight" onClick={(e) => e.stopPropagation()}>
                      <div className="w-full text-xs font-semibold text-foreground transition-colors">Stats Summary Only</div>
                      <p className="text-[10px] text-muted-foreground font-normal">Calculate category item counts inside database only</p>
                    </Label>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-3">
            <Button
              ...
              className="w-full h-10 border border-border hover:border-border bg-background/40 text-zinc-700 dark:text-zinc-300 font-semibold text-xs rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-muted cursor-pointer"
            >
          ...
          {diagData && (
            <div className="mt-4 p-4 rounded-xl border border-border bg-background/60 backdrop-blur-md shadow-inner text-zinc-700 dark:text-zinc-300 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-900 text-muted-foreground font-semibold border border-border">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3.5">
                <div className="p-2.5 rounded-lg bg-card border border-border">
                  <div className="text-[10px] text-muted-foreground font-medium">Estimated Unique Products</div>
                ...
                <div className="p-2.5 rounded-lg bg-card border border-border">
                  <div className="text-[10px] text-muted-foreground font-medium">Multi-Category Sum</div>
                  <div className="text-lg font-bold text-foreground font-mono mt-0.5">
              ...
              <div className="mb-3.5">
                <div className="flex justify-between items-center text-[10px] mb-1">
                  <span className="text-muted-foreground">Multi-Category Product Overlap Rate</span>
                  <span className="font-semibold text-amber-400 font-mono">{diagData.overlap_percentage}%</span>
                </div>
                <div className="w-full h-1.5 bg-zinc-100 dark:bg-zinc-900 rounded-full overflow-hidden border border-border">
                  <div 
              ...
              <div>
                <span className="text-[9px] text-muted-foreground font-bold block mb-1.5 uppercase tracking-wide">Category Distribution Breakdown</span>
                <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
                  {diagData.categories.map((cat: any) => (
                    <div key={cat.slug} className="flex justify-between items-center text-[10px] bg-card px-2.5 py-1.5 rounded-md border border-border">
                      <div className="flex flex-col">
                        <span className="font-medium text-zinc-700 dark:text-zinc-300">{cat.name_en}</span>
                        <span className="text-[8px] text-muted-foreground font-mono">{cat.slug}</span>
                      </div>
      ...
      <div className="lg:col-span-2 flex flex-col gap-6">
        ...
        {(() => {
          ...
          return (
            <div className={cn("grid gap-4 grid-cols-1 sm:grid-cols-2", latestJob ? "xl:grid-cols-4" : "xl:grid-cols-2")}>
              ...
              {latestJob && (
                <div className="p-4 rounded-xl bg-card border border-border shadow-md flex flex-col justify-between">
                  <div>
                  ...
                  <div className="w-full bg-background h-1.5 rounded-full mt-3.5 overflow-hidden">
                    <div
              ...
              {latestJob && (
                <div className="p-4 rounded-xl bg-card border border-border shadow-md flex flex-col justify-between">
                  ...
                  <span className="text-[9px] text-muted-foreground mt-3.5 block font-mono">
              ...
              <div className="p-4 rounded-xl bg-card border border-border shadow-md flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300 font-mono capitalize">
                      {latestJob?.status || "Idle / Offline"}
                    </span>
                  </div>
                </div>
                <span className="text-[9px] text-muted-foreground mt-3.5 block truncate font-mono" title={currentAction}>
                  {currentAction}
                </span>
              </div>
              ...
              <div className="p-4 rounded-xl bg-card border border-border shadow-md flex flex-col justify-between">
                <span className="text-[10px] uppercase font-semibold text-muted-foreground block">Actions control</span>
                {latestJob && (latestJob.status === "running" || latestJob.status === "pending") ? (
                  ...
                  <span className="text-xs text-muted-foreground italic mt-2 block">No campaign running</span>
                )}
              </div>
            </div>
          );
        ...
        <div className="flex-1 min-h-[380px] p-5 rounded-2xl bg-background border border-border shadow-2xl flex flex-col">
          <div className="flex items-center justify-between pb-3 border-b border-border mb-3">
            <div className="flex items-center gap-2">
              <Terminal className="w-4 h-4 text-primary animate-pulse" />
              <span className="text-xs font-bold text-muted-foreground font-mono">Live Telemetry Terminal</span>
            </div>
            {activeJobId && (() => {
              const activeJob = jobs.find((j) => j.status === "running" || j.status === "pending");
          ...
          <div ref={terminalRef} className="flex-1 overflow-y-auto max-h-[360px] font-mono text-[11px] leading-5 space-y-1 text-zinc-700 dark:text-zinc-300 pr-2">
            {logs.length === 0 ? (
              ...
              logs.map((log, i) => (
                <div key={i} className="whitespace-pre-wrap select-all">
                  <span className="text-muted-foreground/70 mr-2">[{log.timestamp.slice(11, 19)}]</span>
